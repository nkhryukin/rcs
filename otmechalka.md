...
- Сделка. К каждому контакту привязываются сделки, например, покупка абонементов, участие в турнирах и т.д.
...
![Верхнеуровневая диаграмма](images/otmechalka.drawio.svg "Верхнеуровневая диаграмма")
...
Не все продукты соответствуют абонементам в отмечалке. Нужные продукты лежат в каталоге "Индивидуальные занятия" (id=480) и "Групповые занятия" (id=478). Чтобы получить каталог товара нужно запросить его продукт в API `crm.product.get` (связь через поле `PRODUCT_ID` в товаре). В полученном продукте нужно смотреть на поле `SECTION_ID`.

## Алгоритмы интеграции

### Передача клиентов в отмечалку. Happy Case

1. В IS по расписанию запускается периодическая задача SYNC_JOB.
2. SYNC_JOB через API `crm.contact.list` запрашивает из B24 всех клиентов, которые (должны быть выполнены все условия):
...
4. SYNC_JOB преобразует поля контакта из формата B24 в формат отмечалки согласно [маппингу](#маппинг-полей-контакта). Если продукты и филиалы отличаются от тех, которые описаны в маппинге, то они игнорируются.
5. SYNC_JOB вызывает API отмечалки для поиска клиента (TODO какое?). В качестве параметра используется `ID` из B24. (TODO если ID не удастся передать в отмечалку, то придется искать по телефону, причем перебором, т.к. в битриксе это массив).
6. Если клиент не найден, то SYNC_JOB вызывает API отмечалки для создания нового клиента (TODO какое?).
7. Если клиент найден, то SYNC_JOB вызывает API отмечалки для обновления клиента (TODO какое?).
...
SYNC_JOB вызывает API в B24 и сохраняет в поле "Статус синхронизации" проблемного объекта текст ошибки.
...
- Как понять, что товар в сделке относится именно к абоменементам, а турнир или что-то еще? - Смотреть на папку, где лежит товарное предложение.
