...
9. Если сделка не найдена, то DEAL_SYNC_JOB вызывает API отмечалки для создания нового абонемента (TODO какое?).
10. Если сделка найдена, то обработка сделки завершается, переход к следующей сделке из списка.
...
- Как понять, что товар в сделке относится именно к абоменементам, а турнир или что-то еще?
...
- Какая доступность API отмечалки (% uptime)
